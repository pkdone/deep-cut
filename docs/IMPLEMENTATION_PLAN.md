...
| **Playback** | Spotify: **Web API (remote device)** default, **Web Playback SDK** optional ([ADR-004](adr/004-spotify-playback-strategy.md)); volume/mute semantics and cross-source pause ([ADR-005](adr/005-playback-volume-mute-and-cross-source-handoff.md)); cross-source handoff acceptable; track failure → local fuzzy fallback → error → skip |
